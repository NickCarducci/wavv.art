...
class Made extends React.Component {
  ...
  removeMessage(e) {
    this.setState({ messages: [] }, () => {
      deleteDoc(doc(firestore, "messages", e))
        .then((ref) => {
  ...
  handleSubmitEvent = () => {
    ...
    if (answer === true) {
      if (isEvents) {
        ...
        const geocollection = GeoFirestore.collection(coll);
        ...
        updateDoc(doc(firestore, coll, this.state.chosenEntity.id), {
          chosenPhoto: this.state.chosenPhoto ? this.state.chosenPhoto : null,
          message: chosenEntity.message,
          body: chosenEntity.body,
          ratings: chosenEntity.ratings,
  ...
  onDeleteClub = (id) => {
    console.log(id);
    deleteDoc(doc(firestore, "clubs", id));

    getDocs(
      query(collection(firestore, "messages"), where("eventroom", "==", id))
    ).then((querySnapshot) => {
  ...
  onDeleteEvent = (id) => {
    console.log(id);
    deleteDoc(doc(firestore, "planner", id));
  ...
  render() {
    ...
    if (chosenEntity) {
      ...
      var coll = this.props.eventsInitial
        ? "planner"
                      ...
                      <div
                        onClick={() => {
                          ...
                          var entityCo = chosenEntity.communityId
                            ...
                            : chosenEntity.city;
                          var entityType = coll;
                          var threadId = coll + entityId + recipients;
                          var x = {
                            threadId,
                            recipients,
                            entityId,
              ...
              {!this.props.classInitial &&
                ...
                !this.props.housingInitial && (
                  <Link
                    ...
                    to={`/sd/${coll}/${chosenEntity.id}`}
                  >
                    Schedule /sd/{coll}/{chosenEntity.id}
                  </Link>
                )}
              <div
                    ...
                    <Link
                      to={
                        coll === "classes"
                          ...
                          : ["housing", "jobs", "planner"].includes(coll)
                          ? `/${coll === "planner" ? "events" : coll}/${x.id}`
                          : `/${coll}/${
                              thiscommunity
                                ? thiscommunity.message
                                : encodeURIComponent(x.city)
