...
class EditNotePage extends React.PureComponent {
  ...
  doit = (y) => {
    onSnapshot(doc(firestore, "chats", y), (doc) => {
      if (doc.exists()) {
        var foo = doc.data();
        foo.id = doc.id;
        if (foo.message !== "") {
          ...
          if (w === [leg].length) {
            ...
            entityIds.forEach(
              (fo) =>
                fo.entityType !== "users" &&
                onSnapshot(
                  doc(firestore, fo.entityType, fo.entityId),
                  (doc) => {
                    if (doc.exists()) {
  ...
  async handleSave() {
    ...
    if (chat && this.props.auth.uid === chat.authorId) {
      ...
      updateDoc(doc(firestore, "chats", this.state._id), {
        topic: "*",
        entityType: "users",
        entityId: null,
        ...
        .then((docRef) => {
          updateDoc(doc(firestore, "calendar", docRef.id), {
            authorId: this.props.auth.uid,
                        ...
                        >
                          {this.state.recipients.includes(x.id) ? (
                            <div
                              ...
                              onClick={() => {
                                updateDoc(
                                  doc(firestore, "chats", this.state._id),
                                  {
  ...
  event.d
    ...
    : */
                ...
                ? () => {
                    ...
                    updateDoc(doc(firestore, "chats", this.state._id), {
                      ...
                      .then((ref) => {
                        ...
                        deleteDoc(doc(firestore, "calendar", ref.id));
                      })
              ...
              this.props.auth === undefined
                ...
                : () => {
                    ...
                    updateDoc(doc(firestore, "users", this.props.auth.uid), {
                      backedUpCount: firebase.firestore.FieldValue.increment(1)
                    ...
                    addDoc(collection(firestore, "planchats"), {
                      firstChat: true,
                      authorId: this.props.auth.uid,
        ...
        <div
          style={{
            bottom: "0px",
            display: this.state.editPhoto ? "flex" : "none",
            position: "relative",
            textAlign: "center",
