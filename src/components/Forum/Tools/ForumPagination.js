...
import { profileDirectory } from "../../../App/Community/arraystrings";
...
class ForumPagination extends React.Component {
        ...
        await new Promise((resolve, reject) => {
          ...
          if (paginate) {
            if (this.state[type[paginate]]) {
              if (paginate === "undo") {
                close = [
                  collection(firestore, type.commentsName),
                  where(
                    "messageAsArray",
                    "array-contains",
  ...
  handleProfileComments = (profile, paginate, queryWord) => {
    ...
    Promise.all(
      ...
      commms.map((post) =>
        onSnapshot(
          doc(collection(firestore, post.collection), post.forumpostId),
          async (doc) => {
            if (doc.exists) {
              var foo = doc.data();
              foo.id = doc.id;
  ...
  capture = (coll, queryWord) => {
    ...
    this.setState(
      ...
      () => {
        const colle = coll !== "forum" ? "entity" : coll;
        this.props.onlyPost();
        if (!coll && this.props.swipe === "comments" && this.props.profile) {
          this.getComments(this.props.profile, queryWord);
        } else {
          if (!coll) return console.log("capture coll", coll);
          var snap = false;
            ...
            if (community) {
              snap = [
                ...
                this.props.commtype === "forum"
                  ...
                  : where("commtype", "==", this.props.commtype),
                where("communityId", "==", community.id),
                orderBy("time", "desc"),
                limit(5)
              ];
            } else {
              console.log("querying " + this.props.city + " for " + queryWord);
              snap = [
                collection(firestore, colle),
                where("messageAsArray", "array-contains", queryWord),
              ...
              if (querySnapshot.empty) {
                ...
                querySnapshot.docs.forEach(async (doc) => {
                  ...
                  if (doc.exists) {
                    var foo = doc.data();
                    foo.id = doc.id;
                    foo.collection = colle;
                    foo.shortId = "forum" + doc.id;
                    foo.droppedPost =
                      foo.droppedId &&
                  ...
                  if (querySnapshot.docs.length === p) {
                    //console.log("queryTermResults", queryTermResults);
                    if (!this.props.profile) {
                      //console.log("forumPosts", queryTermResults);
                      this.props.setData({
      ...
      this.setState({ lastQueryWord: queryWord }, () => {
        this.queryTermTimeout = setTimeout(() => {
          ...
          if (
            ...
            let o = 0;
            profileDirectory.forEach((type) => {
              o++;
              return this.capture(type.collection, queryWord);
            });
            if (o === profileDirectory.length) {
              //this.props.unloadGreenBlue();
            }
          } else {
