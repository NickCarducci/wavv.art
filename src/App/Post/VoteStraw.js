...
class VoteStraw extends React.Component {
  ...
  handleChangeSupport = (parent, way, candidate) => {
    if (this.state.support) {
      updateDoc(doc(firestore, "supports", this.state.support.id), {
        reference:
          parent.electionType +
          "/" +
          parent.message +
  ...
  handleTrue = (candidateId) => {
    if (
      ...
      return updateDoc(doc(firestore, "candidates", candidateId), {
        upvotes: firebase.firestore.FieldValue.arrayRemove(this.props.auth.uid)
      });
    } else return null;
  };
                ...
                (this.props.community.admin &&
                  ...
                  <form
                    onSubmit={(e) => {
                      ...
                      updateDoc(
                        doc(firestore, "elections", this.props.parent.id),
                        {
