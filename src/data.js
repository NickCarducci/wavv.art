...
class Data extends React.Component {
  ...
  fetchForum = (city, commtype, noLoad) => {
    if (this.state.lastcity === city) return null; //should be unnecessary because of fuffer
    const foundlast = this.state.freedocs.find((x) => {
      console.log(x);
      return x;
    });
    ...
    if (["new", "lessons", "show", "game"].includes(this.props.commtype)) {
      if (["new", "lessons", "show", "game"].includes(commtype)) {
        ...
        console.log(
          commtype + " (is the) same query as [new, lesson, show, game] for now"
        );
      }
    }
    this.setState(this.newPostingsClass);
    ...
    await Promise.all(
      hp.map(async (f, i) => {
        ...
        return await new Promise(async (resolve) => {
          ...
          if (["elections", "oldElections"].includes(coll)) {
          ...
          } else if (["oldCases", "cases"].includes(coll)) {
            const {
              prosecution,
              defense,
              jury,
      ...
      entity: async () => {
        if (!recordedEntityNames.includes(name + communityNameUnparsed)) {
          ...
          return await new Promise(async (resolve, reject) => {
            ...
            getDocs(
              query(
                collection(firestore, entityCollection),
                where("messageLower", "==", name.toLowerCase())
              )
            )
              .then((querySnapshot) => {
    ...
    return {
      entity: async () => {
        if (!recordedEntities.includes(entityType + entityId)) {
          ...
          var close = onSnapshot(
            doc(firestore, entityType, entityId),
            (async (doc) => {
              if (doc.exists()) {
                var entity = doc.data();
                entity.id = doc.id;
          ...
          return await new Promise(async (resolve, reject) => {
            !fine && reject(!fine);
            getDoc(doc(firestore, entityType, entityId))
              .then(async (doc) => {
                if (doc.exists()) {
                  var entity = doc.data();
                  entity.id = doc.id;
  ...
  paginateProfilePosts = async (type, profile, way, i) => {
    ...
    if (!this.state[type[way]]) {
      ...
      if (way === "last") {
        ...
        fbbb = query(
          collection(firestore, type.collection),
          where("authorId", "==", profile.id)
        );
      } else {
  ...
  getPosts = (profile) => {
    ...
    const copy = profileDirectory.map((type) => {
      ...
      const jailclass = {
        ...
        //for each: foo = {...doc.data(),doc.id}
        snapshotQuery: [
          collection(firestore, type.currentCollection),
          where("authorId", "==", profile.id)
        ], //optional canIncludes()?
        keepalive,
        sort: { order: "time", by: "desc" }, //sort
    ...
    this.setState({
      jailclasses: [
        ...this.state.jailclasses.filter((x) =>
          copy.map((y) => "getPosts" + y.currentCollection !== x.uuid)
        ),
        ...copy
      ]
    });
  ...
  fetchEvents = async (location, distance, city, targetid) => {
    ...
    const jailclass = {
      ...
      snapshotQuery: this.GeoFirestore.collection(coll), //optional canIncludes()?
      keepalive,
  ...
  getProfileEntities = (profile) => {
    ...
    if (q === types.length + types.length) {
      this.setState({
        jailclasses: [
          ...this.state.jailclasses.filter(
            (x) => copy.map((y) => x.uuid !== y.uuid) // "getEntityQuery" + y.coll + y.role)
          ),
          ...copy
        ]
      });
  ...
  getEntityQuery = (coll, role, profile, type) =>
    /*this.setState({
      jailclasses: [
        ...this.state.jailclasses.filter(
  ...
  handleProfileComments = (profile, paginate) => {
    ...
    Promise.all(
      ...
      Promise.all(
        ...
        console.log("profileComments", profilePosts);
        //console.log(old);
        this.props.setForumDocs({
          profileComments: profilePosts // [...old, ...profilePosts]
